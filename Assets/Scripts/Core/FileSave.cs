...
public class FileSave : MonoBehaviour
{
	...
	private void EncodeSetup(bool clipboard)
	{
		...
		tex = new Texture2D(cardSize.x, cardSize.y, TextureFormat.RGBA32, false, true);
		...
		//Graphics.CopyTexture(screenRender, 0, 0, (int)(source.anchoredPosition.x * sizeMultiplier), (int)(source.anchoredPosition.y * sizeMultiplier), cardSize.x, cardSize.y, tex, 0, 0, 0, 0);
	...
	private IEnumerator EncodeMultipleIE(List<ListItem> set)
	{
		...
		// pre-emptively set active to first to avoid gradient/texture setup bug timing
		setManager.SetNewActiveItem(setCopy[0], false);
		...
		for (int i = 0; i < setCopy.Count; i++)
		{
			...
			// DETERMINE NAME
			string fileName = (i + 1) + " " + setCopy[i].cardData.cardName;
			fileName = GetUseableFileName(fileName);
	...
	private IEnumerator EncodeCompImagesIE(List<ListItem> set, bool clipboard = false)
	{
		...
		int emptyCardSlots = gridLayout.x * gridLayout.y - Mathf.Min(setCopy.Count, compLayouts.Length - 1);
		...
		for (int i = 0; i < setCopy.Count && i < compLayouts.Length - 1; i++)
		{
			...
			if (rowInverse == 0)
			{
				...
				//Graphics.CopyTexture(tex, 0, 0, 0, 0, cardSize.x, cardSize.y, comp, 0, 0, column * cardSize.x + x_offset, rowInverse * cardSize.y);
			}
			else
			{
				comp.SetPixels(column * cardSize.x, rowInverse * cardSize.y, cardSize.x, cardSize.y, tex.GetPixels(0, 0, cardSize.x, cardSize.y));
				//Graphics.CopyTexture(tex, 0, 0, 0, 0, cardSize.x, cardSize.y, comp, 0, 0, column * cardSize.x, rowInverse * cardSize.y);
			}
			Destroy(tex);
		}
