...
public class CardCode : MonoBehaviour
{
	...
	[SerializeField] private ControlSpellSpeed nullSpeedToggler;
	...
	private void DoReadInput()
	{
		...
		{
			code += $"Level Up: {levelUp.value.TrimEnd(' ')}";
		}
	...
	public void WriteInput(string originalInputString)
	{
		...
		if (creditIndex > 0)
		{
			...
			creditF.text = creditString.TrimEnd(' ');
		...
		if (cardTextStartIndex > 0 && cardTextEndIndex > 0)
		{
			...
			cardTextString = cardTextString.Substring(1, cardTextString.Length - 2).TrimEnd(' ');
		...
		if (!CardType.IsSpell(cardType.value))
		{
			...
			if (levelUpIndex > 0)
			{
				...
				levelUp.value = levelUpSubstring.TrimEnd(' ');
				levelUpTextF.SetTextWithoutNotify(levelUpSubstring.TrimEnd(' '));
			}
			else
			{
				levelUp.value = "";
		...
		if (working.Contains("#"))
		{
			...
			groupSubstring = groupSubstring.TrimEnd(' ');
